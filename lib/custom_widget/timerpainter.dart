...
class TimerPainter extends CustomPainter with ChangeNotifier {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Paint timerCirclePaint =
        _buildPaintObj(HexColor('#8641CC'), PaintingStyle.stroke, strokeWidth: 5.0);
    Paint timerPaint = _buildPaintObj(Colors.white, PaintingStyle.stroke, strokeWidth: 7.0);
  ...
  _buildPaintObj(Color color, PaintingStyle paintingStyle,
      {double strokeWidth = 0.0, BlendMode blendMode = BlendMode.srcOver, StrokeCap strokeCap = StrokeCap.round}) {
    var paint = Paint()
      ..color = color
      ..style = paintingStyle
      ..strokeCap = strokeCap
  ...
  _drawText(Canvas canvas) {
    ...
    tp.layout();
    tp.paint(canvas, Offset((parentHeight! / 2) - tp.width / 2, (parentWidth! / 2) - tp.width / 2));
  ...
  _drawTimerCircle(Paint paint, Canvas canvas) {
    ...
    var rect = Rect.fromLTRB(paint.strokeWidth, paint.strokeWidth,
      ...
      canvas.drawArc(
          rect, _getRadianValue(), _getRadianValue(360), false, paint);
  ...
  _drawTimerProgress(Paint paint, Canvas canvas){
    var rect = Rect.fromLTRB(5, 5,
        parentWidth! -  5, parentHeight! - 5);
